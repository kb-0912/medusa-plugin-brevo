...
exports.default = (container) => {
    ...
    app.get("/export", async (req, res) => {
        var _a, _b;
        const pdfGen = new pdfGenerator_1.default();
        ...
        for (const orderId of orderIds) {
            ...
            try {
                if (((_b = (_a = options === null || options === void 0 ? void 0 : options.pdf) === null || _a === void 0 ? void 0 : _a.enabled) !== null && _b !== void 0 ? _b : false) && pdfGen && pdfGen.createInvoice) {
                    const base64 = await pdfGen.createInvoice(options, data);
                    // attachments.push({
                    //     name: "invoice.pdf",
                    //     base64,
                    ...
                    const dir = path_1.default.join(__dirname, '..', '..', 'pdf');
                    if (!fs_1.default.existsSync(dir)) {
                        fs_1.default.mkdirSync(dir);
                    }
                    console.log(dir);
                    const filename = path_1.default.join(dir, orderId + '.pdf');
                    fs_1.default.writeFile(filename, Buffer.from(base64, 'base64'), (err) => {
                        if (err)
                            throw err;
    ...
    const getOrderData = async (orderService, totalsService, orderId) => {
        ...
        const items = await Promise.all(order.items.map(async (i) => {
            ...
            i.discounted_price = `${humanPrice_(i.totals.total / i.quantity, currencyCode)} ${currencyCode}`;
            i.price = `${humanPrice_(i.totals.original_total / i.quantity, currencyCode)} ${currencyCode}`;
            return i;
        }));
        let discounts = [];
        if (order.discounts) {
        ...
        return {
            ...
            items,
            discounts,
            subtotal_ex_tax: `${humanPrice_(subtotal_ex_tax, currencyCode)} ${currencyCode}`,
            subtotal: `${humanPrice_(subtotal, currencyCode)} ${currencyCode}`,
            gift_card_total: `${humanPrice_(gift_card_total, currencyCode)} ${currencyCode}`,
            tax_total: `${humanPrice_(tax_total, currencyCode)} ${currencyCode}`,
            discount_total: `${humanPrice_(discountTotal, currencyCode)} ${currencyCode}`,
            shipping_total: `${humanPrice_(shipping_total, currencyCode)} ${currencyCode}`,
            shipping_total_inc: `${humanPrice_(((_a = order === null || order === void 0 ? void 0 : order.shipping_methods[0]) === null || _a === void 0 ? void 0 : _a.price) || shipping_total, currencyCode)} ${currencyCode}`,
